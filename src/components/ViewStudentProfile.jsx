...
const ViewStudentProfile = ({ student, onClose, onStudentUpdated, onStudentDeleted }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center">
      <div className="bg-white w-full max-w-3xl rounded-xl p-6 overflow-y-auto max-h-[95vh] shadow-lg">
        {/* Header */}
        <div className="flex justify-between items-center mb-4 border-b pb-2">
          <h2 className="text-lg font-bold text-blue-700">
            {studentData.name} - {studentData.enrollment}
          </h2>
          <button onClick={onClose} className="text-xl font-bold text-red-600 hover:scale-110 transition">
            ✕
          </button>
        </div>
